...
Class ADVERTISMENT
{
     ...
     public static function advertisment_Insert($name, $state)
    {
     ...
     public static function advertisment_UpdateRowByState($name, $state)
    {
        $sql = 'CALL `sp_advertisment_UpdateRowByState`(:name, :state)';
        $params = array (':name' => $name, ':state' => $state);
        DatabaseHandler::Close(); 
        return DatabaseHandler::Execute($sql, $params);
    }
     public static function advertisment_SelectRowByState($state)
    {
        $sql = 'CALL `sp_advertisment_SelectRowByState`(:state)';
